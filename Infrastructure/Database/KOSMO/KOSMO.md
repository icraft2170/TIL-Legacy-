...
SELECT *
FROM EMP
WHERE JUMIN_NUM LIKE '7_____1%' OR JUMIN_NUM LIKE '6_____1%';
...
SELECT EMP_NAME,EMP_NO,SYSDATE - HIRE_DATE,MONTHS_BETWEEN(SYSDATE,HIRE_DATE),ADD_MONTHS(HIRE_DATE,5),HIRE_DATE, LAST_DAY(HIRE_DATE),NEXT_DAY(HIRE_DATE ,'일'),round(SYSDATE -HIRE_DATE,1)
FROM EMP;
...
SELECT *
FROM CUSTOMER
WHERE EMP_NO NOT IN(9) OR EMP_NO IS NULL;


SELECT EMP_NAME , CEIL(SALARY + SALARY * 0.05)
FROM EMP
WHERE 3000 <= (SALARY + SALARY * 0.05)
...
SELECT *
FROM EMP
WHERE HIRE_DATE BETWEEN TO_DATE('1990','RRRR') AND TO_DATE('19991231','RRRRMMDD') ;

SELECT TO_DATE('1990','RRRR')
FROM emp

SELECT *
FROM emp
WHERE DEPT_NO in(10,30) AND SALARY <= 3000 AND HIRE_DATE < TO_DATE('1996-01-01'); 
...
SELECT *
FROM emp
WHERE emp NOT LIKE '황%';

SELECT *
FROM emp
WHERE SUBSTR(emp_name,LENGTH(emp_name),1) = '동';
...
SELECT *
FROM emp
WHERE (LENGTH(emp_name)=3) AND SUBSTR(EMP_NAME,1,1) = '김'
...
SELECT *
FROM emp
WHERE EMP_NAME LIKE '_%김%_' AND EMP_NAME NOT LIKE '김%' AND EMP_NAME NOT LIKE '%김' ;

SELECT *
FROM emp
WHERE JUMIN_NUM LIKE '______2%'
OR JUMIN_NUM LIKE '______4%';

SELECT *
FROM EMP
WHERE JUMIN_NUM LIKE '6_____1%' 
...
SELECT c.CUS_NO ,c.CUS_NAME,e.EMP_NAME
...
SELECT c.CUS_NO ,c.CUS_NAME,e.EMP_NAME 
FROM EMP e FULL OUTER JOIN CUSTOMER c
ON (e.EMP_NO =c.EMP_NO );
...
SELECT c.CUS_NO ,c.CUS_NAME,e.EMP_NAME 
FROM EMP e RIGHT OUTER JOIN CUSTOMER c
ON (e.EMP_NO =c.EMP_NO );

SELECT e.EMP_NAME,e.SALARY,sg.SAL_GRADE_NO
